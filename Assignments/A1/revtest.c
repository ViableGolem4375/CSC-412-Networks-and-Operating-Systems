...
int main(int argc, char** argv) {
  ...
  */
  ...
  // Check to see if there are no arguments passed in.
  // If so, print USAGE and run tests.
  if (argc == 1) {
    ...
    // Check to see if there is one argument passed.
    // If so, print USAGE and run tests.
    if (num_elements == 1) {
      fprintf(stdout, USAGE);
      test_array_reverse(&reverse_arr1,1);
    ...
    // Check for bad input.
    // If so, print ERRORMSG and return EXIT_FAILURE.
    else if (num_elements != argc - 2) {
      fprintf(stderr, ERRORMSG);
      return EXIT_FAILURE;
    }
    else {
        // Initialize an array of character pointers
        ...
        // Print the reversed array to stdout
        for (int i = 0; i < num_elements; i++) {
            printf("%s ", arr[i]);
        }
        printf("\n");
