...
int main(int argc, char** argv) {
    ...
    char name_buffer[1000]; // Create a buffer to store the user input for name.
    read_name(name_buffer); // Call read_name and read in the user input.

    initialize_window(width, height); // Call initialize window here.
    while (g_game_over == 0) {
        // Step 1: Wait for some specified amount of time.
        usleep(100000); // 100 milliseconds.

        // Step 2: Retrieve user input.
        enum input_key input = get_input();

        // Step 3: Update the game state based on user input.
        update(cells, width, height, NULL, input, 0);

        // Step 4: Render the new game state to the screen.
        render_game(cells, width, height);
    }

    // Call end_game when the above loop terminates.
